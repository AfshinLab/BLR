...
system( sprintf("bedtools merge -i %s -c 4,5,6,7,8,9 -o first,collapse,max,mean,first,count -d %d > %s", bedfile,max_distance,bedfile_merged) )

dd <- read.table(bedfile_merged)
write.table(bed_to_bedpe(dd), file = bedpefile_merged ,sep = '\t', row.names = F, col.names = F, quote = F)

system( sprintf("SURVIVOR bedpetovcf %s %s",bedpefile_merged,vcffile_merged))
system( sprintf("rm %s %s",bedfile,bedfile_merged))
